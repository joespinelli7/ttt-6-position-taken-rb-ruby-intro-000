require_relative '../lib/position_taken2.rb'

describe '#position_taken2? in lib/position_taken.rb' do
  it 'accepts a board and the index to check as arguments' do
    board = []
    index = 0

    expect{position_taken2?(board, index)}.to_not raise_error
  end
  ...
  it 'returns false if the board has an empty string " " in the desired index' do
    ...
    expect(position_taken2?(board, index)).to be(false)
  end
  ...
  it 'returns true if the board has a value of "X" or "O" in the index' do
    ...
    index = 0
    expect(position_taken2?(board, index)).to be(true)
